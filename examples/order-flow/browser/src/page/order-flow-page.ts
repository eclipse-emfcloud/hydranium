...
import { DataEvents, DataSession } from '@hydranium/protocol';
import {
   type DomainModel,
   isLayoutModel,
   type LayoutModel,
...
import { requireButton, requireCheckbox, requireElement } from './dom.js';
...
/**
 * Read one document through the DATA head and report what it says about it.
 *
 ...
 */
async function readThroughDataHead({ session }: DataHead): Promise<string> {
   await session.openDocument(DATA_HEAD_DOCUMENT);
   // `openDocument` settles at the integrity landmark, where the diagnostics
   // array is empty BY PHASE CONTRACT rather than because the document is
   // valid — so a client that adopts the open snapshot and never asks again
   // reports a clean document forever. Asking explicitly is the documented way
   // to get a validated one, and the VS Code panel had this exact defect.
   const server = await session.connected();
   const document = await server.getModelDocument({ uri: DATA_HEAD_DOCUMENT, includeDiagnostics: true });
...
async function main(): Promise<void> {
   // Before anything else, so the page never paints in one scheme and then
   // flips: the editors read the current scheme when they are created, and the
   // chrome's roles are resolved as soon as the attribute is set.
   wireSchemeSwitch();
   ...
   requireElement('data-head').textContent = await readThroughDataHead(dataHead);
